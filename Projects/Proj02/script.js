...
class Wordle {
     ...
     fetchWord() {
          fetch("https://random-word-api.herokuapp.com/word?length=5")
                ...
                .then(data => {
                     //array returns a single word in an array
                     const word = data[0].toUpperCase();
                     //RegEx validate the string has 5 letters, only letters, no spaces or special characters
                     if (word && /^[A-Z]{5}$/.test(word)) {
                          this.answer = word;
                          //print answer to console for testing
                          console.log("Answer:", this.answer);
                     //if the word from the api is not valid, show an alert
                     } else {
                          alert("Could not load a valid word. Please refresh.");
                          console.error("Invalid word from API:", word);
                     }
                })
                .catch(() => {
     ...
     makeKeyboard() {
        ...
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").forEach(letter => {
            ...
            // Add event listener to handle clicks on the key
