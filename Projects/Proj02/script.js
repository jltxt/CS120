class Wordle {
    ...
    fetchWord() {
        //api call to Datamuse to get 5-letter words
        fetch("https://api.datamuse.com/words?sp=?????&max=100")
            //convert response to JSON
            .then(res => res.json())
            //now it is an array of word objects
            .then(data => {
                //filter to get only valid 5-letter words
                const validWords = data
                    .map(entry => entry.word.toUpperCase())
                    .filter(word => /^[A-Z]{5}$/.test(word));

                if (validWords.length > 0) {
                    //pick a random word from the validWords array
                    const word = validWords[Math.floor(Math.random() * validWords.length)];
                    this.answer = word;
                    console.log("Answer word from Datamuse:", this.answer);
                } else {
                    this.FallbackWord("Invalid word from Datamuse.");
                }
            })
            .catch(() => {
                this.FallbackWord("Failed to fetch from Datamuse. Using fallback words.");
            });
    }
